...
@login_required
def rotate_photo(request, pk):
    """Rotate a photo 90 degrees clockwise or counter-clockwise."""
    photo = get_object_or_404(Photo, pk=pk)
    album = photo.album
    
    # Check permissions
    if not is_album_admin(request.user, album):
        messages.error(request, 'You do not have permission to rotate this photo.')
        return redirect('album:dashboard')
    
    if request.method == 'POST':
        direction = request.POST.get('direction', 'right')
        return_url = request.POST.get('return_url', reverse('album:photo_detail', args=[pk]))
        ...
        try:
            ...
            # Update the photo's image field
            photo.image.save(original_name, ContentFile(output.read()), save=False)
            
            # Update dimensions
            photo.width, photo.height = rotated.size
            photo.save()
            
            # Regenerate thumbnails by accessing the ImageKit fields
            # This forces regeneration of thumbnail, medium, and large versions
            photo.thumbnail.generate()
            photo.medium.generate()
            photo.large.generate()
            
            messages.success(request, f'Photo rotated {direction} successfully.')
            
        except Exception as e:
            logger.error(f"Error rotating photo {pk}: {e}")
            messages.error(request, f'Error rotating photo: {str(e)}')
        
        return redirect(return_url)
    
    return redirect('album:photo_edit', pk=pk)


@login_required
def video_edit(request, pk):
    """Edit video metadata, tags, and custom albums for album admins."""
                ...
                try:
                    ...
                    if f.content_type.startswith('image/') or \
                       (f.content_type == 'application/octet-stream' and file_extension in ['.jpg', '.jpeg', '.png', '.gif', '.webp']):
                        ...
                        file_checksum = calculate_checksum(f)
                        
                        # Check if photo with same checksum already exists in this album
                        ...
                        if existing_photo:
                            ...
                            failed_uploads.append({
                                'name': f.name,
                                'reason': f'Duplicate photo already exists in album (matches: {existing_photo.title})'
                            })
                            continue
                        
                        # Create Photo object with checksum
...
def toggle_video_favorite(request, video_id):
    """Toggle favorite status for a video via AJAX"""
    video = get_object_or_404(Video, pk=video_id)
    
    # Check if user has permission to view the video (through album access)
    album = video.album
    if not (request.user.is_superuser or album.is_public or request.user == album.owner or request.user in album.viewers.all()):
        return JsonResponse({'error': 'Permission denied'}, status=403)
...
def favorites_list(request):
    """Display user's favorited photos and videos"""
    favorites = Favorite.objects.filter(user=request.user).select_related(
        'photo__album', 'video__album'
    ).order_by('-created')
    
    # Separate photos and videos
    photos = [fav.photo for fav in favorites if fav.photo]
    videos = [fav.video for fav in favorites if fav.video]
    
    # Combine them for a mixed media list (optional, depending on template needs)
    media_items = []
    for fav in favorites:
        if fav.photo:
            media_items.append({'type': 'photo', 'item': fav.photo, 'created': fav.created})
        elif fav.video:
            media_items.append({'type': 'video', 'item': fav.video, 'created': fav.created})
    
    return render(request, 'album/favorites.html', {
        'photos': photos,
        'videos': videos,
        'media_items': media_items,
        'favorites_count': len(media_items)
    })
