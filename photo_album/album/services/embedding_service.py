...
# Lazy-load the model to avoid blocking Django startup
_model = None
...
def _get_model():
    ...
    if _model is None:
        try:
            _model = SentenceTransformer("clip-ViT-B-32-multilingual-v1")
            logger.info("Successfully loaded SentenceTransformer model.")
        except Exception as e:
            logger.error(f"Failed to load SentenceTransformer model: {e}")
            _model = False  # Mark as failed to avoid retry
...
def generate_image_embedding(image_path):
    ...
    try:
        ...
        embeddings = model.encode(
            [numpy_image], convert_to_tensor=True, show_progress_bar=False
        )
        ...
        if (
            ...
            logger.error(
                f"Model returned unexpected output for {image_path}. Shape: {embeddings.shape}, Type: {type(embeddings)}"
            )
            return None
