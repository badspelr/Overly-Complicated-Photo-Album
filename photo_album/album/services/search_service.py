...
class MediaSearchService:
    ...
    def search_album_by_text(query, album_id, user, search_type='text'):
        ...
        if search_type == 'ai':
            ...
            # First, try text search on AI descriptions and user tags for exact matches
            photo_text_matches = photos.filter(
                Q(ai_description__icontains=query) |
                Q(ai_tags__contains=[query]) |  # AI-generated tags
                Q(tags__name__icontains=query)  # User-created tags
            ).distinct()
