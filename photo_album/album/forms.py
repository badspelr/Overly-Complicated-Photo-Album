...
class AlbumForm(forms.ModelForm):
    ...
    def clean_title(self):
        ...
        # Check if an album with this title already exists for this user
        if self.user:
            # Get existing albums with this title for this user
            existing = Album.objects.filter(owner=self.user, title=title)
            
            # If editing an existing album, exclude it from the check
            if self.instance and self.instance.pk:
                existing = existing.exclude(pk=self.instance.pk)
            
            if existing.exists():
                raise forms.ValidationError(
                    f'You already have an album named "{title}". Please choose a different name.'
                )
...
class PhotoForm(forms.ModelForm):
    filename = forms.CharField(
        required=False,
        max_length=255,
        help_text="Change the filename (without extension). Leave empty to keep current filename."
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk:
            ...
            if self.instance.image:
                import os
                current_filename = os.path.basename(self.instance.image.name)
                filename_without_ext = os.path.splitext(current_filename)[0]
                self.fields['filename'].initial = filename_without_ext
    ...
    def save(self, commit=True):
        ...
        new_filename = self.cleaned_data.get('filename', '').strip()
        
        # Handle file renaming if a new filename is provided
        if new_filename and instance.pk and instance.image:
            ...
            if old_path != new_path and os.path.exists(old_path):
                ...
                instance.image.name = os.path.join(os.path.dirname(old_name), new_name)
...
class VideoForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk:
            ...
            if self.instance.video:
                import os
                current_filename = os.path.basename(self.instance.video.name)
                filename_without_ext = os.path.splitext(current_filename)[0]
                self.fields['filename'].initial = filename_without_ext
    ...
    def save(self, commit=True):
        ...
        # Handle file renaming if a new filename is provided
        if new_filename and instance.pk and instance.video:
            ...
            if old_path != new_path and os.path.exists(old_path):
                ...
                # Update the model field
                instance.video.name = os.path.join(os.path.dirname(old_name), new_name)
