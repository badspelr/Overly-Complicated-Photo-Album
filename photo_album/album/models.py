...
class SiteSettings(models.Model):
    ...
    max_album_download_size_mb = models.PositiveIntegerField(
        default=2048,  # 2GB default
        help_text='Maximum album download size in megabytes (MB). Default is 2048 MB (2 GB).'
    )
...
class Album(models.Model):
    ...
    class Meta:
        ...
        ordering = ['-created_at']
        constraints = [
            models.UniqueConstraint(fields=['owner', 'title'], name='unique_album_per_user')
        ]
        indexes = [
...
class PhotoShareLink(models.Model):
    """Share link for individual photos."""
    photo = models.ForeignKey('Photo', on_delete=models.CASCADE, related_name='share_links', help_text='Photo being shared.')
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='created_photo_share_links', help_text='User who created this share link.')
    share_token = models.CharField(max_length=64, unique=True, db_index=True, help_text='Unique token for the share link.')
    expires_at = models.DateTimeField(null=True, blank=True, help_text='When this share link expires.')
    is_active = models.BooleanField(default=True, help_text='Whether this share link is active.')
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"Share link for photo {self.photo.title}"
    ...
    def can_access(self):
        """Check if this share link can be used to access the photo."""
        return self.is_active and not self.is_expired()
...
class Photo(Media):
    image = models.ImageField(upload_to='photos/', max_length=255, help_text='Upload the photo file.')
...
class Video(Media):
    video = models.FileField(upload_to='videos/', max_length=255, help_text='Upload the video file.')
    # Tagging and custom albums
...
class Favorite(models.Model):
    """Model to track user's favorite photos and videos"""
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='favorites')
    photo = models.ForeignKey(Photo, on_delete=models.CASCADE, related_name='favorited_by', null=True, blank=True)
    video = models.ForeignKey(Video, on_delete=models.CASCADE, related_name='favorited_by', null=True, blank=True)
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                ...
                name='favorite_photo_or_video_not_both'
            ),
            models.UniqueConstraint(
                fields=['user', 'photo'],
                name='unique_user_photo',
    ...
    def __str__(self):
        if self.photo:
            ...
            return f"{self.user.username} - Video: {self.video.title}"
        return f"{self.user.username} - Unknown"
    
    @property
    def media_item(self):
        """Return the photo or video object"""
