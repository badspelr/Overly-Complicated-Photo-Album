...
@register.simple_tag
def is_favorited(user, media):
    """
    Check if a photo or video is favorited by the current user.
    ...
    if isinstance(media, Photo):
        return Favorite.objects.filter(user=user, photo=media).exists()
    elif isinstance(media, Video):
        return Favorite.objects.filter(user=user, video=media).exists()
