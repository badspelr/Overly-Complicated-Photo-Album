document.addEventListener('DOMContentLoaded', function() {
    ...
    if (selectModeToggle) {
        selectModeToggle.addEventListener('click', function() {
            ...
            document.body.classList.toggle('select-mode-active', selectModeActive);
            
            if (selectModeActive) {
                selectModeToggle.classList.remove('btn-success');
                selectModeToggle.classList.add('btn-warning');
                selectModeToggle.innerHTML = '<i class="material-icons">close</i> <span>Cancel</span>';
                selectModeToggle.style.backgroundColor = '#ff9800';
                showToast('Selection mode enabled - tap photos to select');
            } else {
                selectModeToggle.classList.remove('btn-warning');
                selectModeToggle.classList.add('btn-success');
                selectModeToggle.innerHTML = '<i class="material-icons">check_circle</i> <span>Select</span>';
                selectModeToggle.style.backgroundColor = '#4caf50';
            }
        });
    }

    // Prevent navigation to detail page when in select mode
    document.addEventListener('click', function(e) {
        if (selectModeActive) {
            const mediaLink = e.target.closest('.media-card a.media-link');
            if (mediaLink) {
                e.preventDefault();
                e.stopPropagation();
                // Find and toggle the checkbox instead
                const mediaCard = mediaLink.closest('.media-card');
                const checkbox = mediaCard.querySelector('input[type="checkbox"].media-checkbox');
