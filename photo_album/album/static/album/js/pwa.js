...
class PWAHandler {
    ...
    if (false && "serviceWorker" in navigator) {
      window.addEventListener("load", () => {
        navigator.serviceWorker
          .register("/static/album/js/sw.js")
          .then((registration) => {
    ...
    switch (data.type) {
      case "offline-queue-updated":
        ...
        if (!window.location.pathname.includes("/upload")) {
          this.showNotification(
            `Offline queue updated. ${data.queueLength} items pending.`,
            "info"
          );
        }
        break;
      case "offline-queue-processed":
        if (data.success) {
          ...
          if (!window.location.pathname.includes("/upload")) {
            this.showNotification(
              "Offline uploads processed successfully!",
              "success"
            );
          }
        }
  ...
  async processOfflineQueue() {
    ...
    if (
      ...
      try {
        ...
        if (!window.location.pathname.includes("/upload")) {
          this.showNotification(
            "Background sync registered for offline uploads.",
            "info"
          );
        }
      } catch (error) {
        ...
        if (e.touches.length === 2) {
          ...
          if (target) {
            initialScale = parseFloat(
              target.style.transform?.match(/scale\(([^)]+)\)/)?.[1] || 1
            );
          }
        }
      },
  ...
  handleSwipe(direction) {
    ...
    const currentImage = document.querySelector(
      ".media-modal img, .media-detail img"
    );
    if (currentImage) {
      // Find next/previous image in gallery
      const images = Array.from(
        document.querySelectorAll(".media-card img, .media-thumbnail img")
      );
  ...
  async requestCameraAccess() {
    try {
      ...
      if (!window.location.pathname.includes("/upload")) {
        this.showNotification(
          "Camera access is required for photo capture.",
          "error"
        );
      }
      return null;
  ...
  async addToOfflineQueue(file, metadata = {}) {
    ...
    if (!this.isOnline()) {
      ...
      if (!window.location.pathname.includes("/upload")) {
        this.showNotification(
          "Photo added to offline queue. Will upload when connection is restored.",
          "info"
        );
      }
    } else {
  ...
  async processOfflineQueue() {
    ...
    if (!window.location.pathname.includes("/upload")) {
      this.showNotification(
        `Processing ${this.offlineQueue.length} offline uploads...`,
        "info"
      );
    }
    ...
    if (window.location.pathname.includes("/upload")) {
      ...
      if (typeof window.showUploadMessage === "function") {
        ...
        const retryCheck = () => {
          ...
          if (typeof window.showUploadMessage === "function") {
            console.log(
              "Using enhanced message system (delayed, attempt",
              retryCount,
              ")"
            );
            window.showUploadMessage(
            ...
            console.log(
              "showUploadMessage still not available after",
              maxRetries,
              "attempts, falling back to PWA notification"
            );
          }
        };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("online", () => {
    // Silently handle reconnection - no notification needed
  });

  window.addEventListener("offline", () => {
    // Don't show notification on upload page - handled by upload page logic
    if (!window.location.pathname.includes("/upload")) {
      window.pwaHandler.showNotification(
