...
(function() {
    ...
    const slideshow = {
        ...
        async loadAllPhotos() {
            ...
            // Build URL to fetch all photos (no pagination, photos only)
            const fetchUrl = new URL(window.location.href);
            fetchUrl.searchParams.set('media_type', 'photos');
            fetchUrl.searchParams.set('page_size', '10000'); // Large number to get all photos
            ...
            try {
                ...
                const mediaCards = doc.querySelectorAll('.media-card[data-media-type="Photo"]');
                this.photos = [];
                ...
                // Fallback to current page photos
        ...
        loadPhotosFromPage() {
            // Get all photo items from the current page (excluding videos) - FALLBACK ONLY
            ...
            console.log(`Loaded ${this.photos.length} photos from current page`);
        },

        async start() {
            // Show loading message
            const slideshowBtn = document.getElementById('slideshow-btn');
            const originalText = slideshowBtn.innerHTML;
            slideshowBtn.innerHTML = '<i class="material-icons">hourglass_empty</i> Loading...';
            ...
            // Restore button
            slideshowBtn.innerHTML = originalText;
