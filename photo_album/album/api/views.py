...
class CreatePhotoShareLinkView(APIView):
    ...
    def post(self, request, photo_id):
        ...
        # Check permission
        if photo.album and photo.album.owner != request.user and not request.user.is_superuser:
            return Response({'error': 'Permission denied'}, status=status.HTTP_403_FORBIDDEN)
        
        from datetime import timedelta
        from django.utils import timezone
        import secrets
        from ..models import PhotoShareLink
        
        # Generate unique token
        share_token = secrets.token_urlsafe(32)
        
        # Get expiration from request (default 7 days)
        expires_days = request.data.get('expires_days', 7)
        expires_at = timezone.now() + timedelta(days=int(expires_days))
        
        # Create share link
        share_link = PhotoShareLink.objects.create(
            photo=photo,
        ...
        # Generate full URL
        share_url = request.build_absolute_uri(f'/share/photo/{share_token}/')
...
class SharedPhotoView(APIView):
    """Access a photo via shareable link (public access, no auth required)."""
    permission_classes = [AllowAny]
    ...
    def get(self, request, token):
        ...
        try:
            share_link = PhotoShareLink.objects.select_related('photo').get(share_token=token)
        except PhotoShareLink.DoesNotExist:
        ...
        if not share_link.can_access():
            if share_link.is_expired():
                ...
                return Response({'error': 'Share link is inactive'}, status=status.HTTP_403_FORBIDDEN)
